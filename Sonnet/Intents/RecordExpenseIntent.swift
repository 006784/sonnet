...
struct RecordExpenseIntent: AppIntent {
    ...
    static var description = IntentDescription("快速记录一笔支出，支持金额、分类和备注")
    static var openAppWhenRun = true

    @Parameter(title: "金额", description: "支出金额")
    var amount: Double

    @Parameter(title: "分类", description: "支出分类", default: "餐饮")
    var category: String
    ...
    static var parameterSummary: some ParameterSummary {
        Summary("记录 \(\.$category) 支出 \(\.$amount) 元") {
            \.$note
        }
    }
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        let dialog = "我先为你写好一笔 \(normalizedCategory) 支出 ¥\(amountStr)，打开十四行诗确认后就会入账。"
        return .result(dialog: "\(dialog)")
    }

    private var normalizedCategory: String {
        let trimmed = category.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? "餐饮" : trimmed
    }
}
